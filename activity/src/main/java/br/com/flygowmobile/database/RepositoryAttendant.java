...
public class RepositoryAttendant extends Repository<Attendant> {
    ...
    protected ContentValues populateContentValues(Attendant attendant) {
        ...
        values.put(Attendants.COLUMN_NAME_REGISTRATION_DATE, parser.format(attendant.getRegistrationDate()));
        values.put(Attendants.COLUMN_NAME_PHOTO_NAME, attendant.getPhotoName());
    ...
    public Attendant findById(long id) {
        ...
        try {
            ...
            if (c.getCount() > 0) {
                c.moveToFirst();
                Attendant attendant = new Attendant(c.getLong(0), c.getString(1), c.getString(2), c.getString(3), parser.parse(c.getString(4)), parser.parse(c.getString(5)), c.getString(6), c.getString(7), c.getString(8), c.getString(9));
                return attendant;
            }
        } catch (Exception e) {
    ...
    public List<Attendant> listAll() {
        try {
            ...
            if (c.moveToFirst()) {
                ...
                int idxRegistrationDate =  c.getColumnIndex(Attendants.COLUMN_NAME_REGISTRATION_DATE);
                int idxPhotoName =  c.getColumnIndex(Attendants.COLUMN_NAME_PHOTO_NAME);
                ...
                do {
                    ...
                    attendant.setRegistrationDate(parser.parse(c.getString(idxRegistrationDate)));

                    attendant.setPhotoName(c.getString(idxPhotoName));
